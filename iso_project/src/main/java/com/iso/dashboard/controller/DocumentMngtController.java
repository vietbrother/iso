...
import com.iso.dashboard.ui.DocumentMngtUI;
import com.iso.dashboard.utils.BundleUtils;
import com.iso.dashboard.utils.ComponentUtils;
import com.iso.dashboard.utils.Constants;
import com.iso.dashboard.utils.DataUtil;
import com.iso.dashboard.utils.ISOIcons;
import com.iso.dashboard.view.DocumentMngtView;
...
public class DocumentMngtController {
    ...
    DocumentMngtView view;
    DocumentMngService service;

    CustomGrid pagedTable;
//    String[] headerName = new String[]{"Id", "Username", "Email", "Phone", ""};
    String prefix = "documentMngt.list";//tien to trong file language
    String headerKey = "header.documentMngt";//lay trong file cas
    ...
    public DocumentMngtController(DocumentMngtView view) {
        this.view = view;
        this.pagedTable = view.getPagedTable();
        initTable(DocumentMngService.getInstance().listUsers(null));
        doAction();
    }

    public void initTable(List<DocumentDTO> lstUsers) {
...
//                        DocumentDTO dto = DocumentMngService.getInstance().getUserById(userId);
//                        onUpdate(dto);
...
//                                            String userId = (String) item.getItemProperty("id").getValue();
//                                            ResultDTO res = DocumentMngService.getInstance().removeUser(userId);
...
//                        view.getBtnSearch().click();
                        ...
                        new ConfirmDialog.Listener() {
                            ...
                            public void onClose(ConfirmDialog dialog) {
                                if (dialog.isConfirmed()) {
                                    // Confirmed to continue
                                    DocumentDTO documentDTO = (DocumentDTO) obj;
                                    ResultDTO res = DocumentMngService.getInstance().removeUser(String.valueOf(documentDTO.getId()));
                                    ComponentUtils.showNotification("Delete id : " + documentDTO.getId() + " " + res.getKey() + " " + res.getMessage());
                                    view.getBtnSearch().click();
                                }
                            }
                        });
    ...
    public void reloadData(List<DocumentDTO> lstUsers) {
        pagedTable.setContainerDataSource(pagedTable.createWrapContainer(createContainer(lstUsers)));
    }

    public IndexedContainer createContainer(List<DocumentDTO> lstUser) {
...
//        });
    ...
    public boolean validateData(DocumentMngtUI ui) {
...
//        }
    ...
    private void onInsert() {
        createDialog(true, new DocumentDTO());
    }

    private void onUpdate(DocumentDTO dto) {
        createDialog(false, dto);
    }

    private void onSearch() {
        List<DocumentDTO> lstUser = DocumentMngService.getInstance().listUsers(view.getTxtUserName().getValue());
...
//        }
//    }
    private void initDataDialog(DocumentMngtUI ui, boolean isInsert, DocumentDTO dto) {
...
//            ComponentUtils.fillDataCombo(ui.getCmbSex(), Constants.EMPTY_CHARACTER, String.valueOf(dto.isMale()), lstSex);
    ...
    public void createDialog(boolean isInsert, DocumentDTO dto) {
        DocumentMngtUI ui = new DocumentMngtUI();
    ...
    private void getDataFromUI(DocumentMngtUI ui, DocumentDTO dto) {
